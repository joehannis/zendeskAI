...
import articleAICall from '../ai/articleAICall.mjs';
...
const fetchArticles = async (obj) => {
  ...
  for (const [tpaKey, tpaValue] of Object.entries(obj)) {
    for (const [tpsaKey, tpsa] of Object.entries(tpaValue.tpsas)) {
      ...
      const initialTokenEstimateResponse = await articleAICall(
        tpaValue.docs,
        tpsa.tickets,
      ...
      for (const [index, chunk] of chunksArray.entries()) {
        ...
        while (attempt < maxAttempts) {
          ...
          try {
            const result = await articleAICall(
              chunk.docs,
              chunk.tickets,
