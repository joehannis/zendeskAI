...
export const addArticleToFirestore = async (
  ...
  try {
    ...
    console.error('Firestore batch write error:', JSON.stringify(e));
    return {
      success: false,
...
export const addDocToFirestore = async (doc, tpa, zendeskId) => {
  if (
    ...
    try {
      ...
      return {
        success: false,
        message: `Firestore batch write failed: ${JSON.stringify(e.message)}`,
      };
    }
  }
};
...
export const docsDelete = async (title) => {
  ...
  try {
    ...
    console.error(
      `Error deleting documents with title "${title}":`,
      JSON.stringify(error)
    );
    return {
      success: false,
      message: `Error deleting documents: ${JSON.stringify(error.message)}`,
    };
  }
};
...
export const appendTicketIds = async (obj, ticketIds) => {
  const snapshot = await firestoreInstance
    .collectionGroup('content')
    .where('zendeskId', '==', obj.zendeskId)
    .get();
  ...
  snapshot.docs.forEach((doc) => {
    ...
    if (Array.isArray(data.ticketIds)) {
      data.ticketIds.forEach((id) => uniqueTicketIds.add(id));
    }
  });

  const ticketIdsArray = [...uniqueTicketIds];
  snapshot.docs.forEach((doc) => {
    batch.update(doc.ref, { ticketIds: ticketIdsArray });
  });

  await batch.commit();
  return {
    success: true,
    message: `Updated ticketIds for ${snapshot.size} documents.`,
  };
};

export const deleteArticlesByZendeskId = async (zendeskIds) => {
  const snapshot = await firestoreInstance
    .collectionGroup('content')
    .where('zendeskId', 'in', zendeskIds)
    .get();
  ...
  return {
    ...
    message: `Deleted ${
      snapshot.size
    } articles with Zendesk IDs: ${zendeskIds.join(', ')}`,
  };
};
