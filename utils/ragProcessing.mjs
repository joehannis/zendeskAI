...
export const processHierarchically = async (originalData, type) => {
  const originalTitle =
    type === 'doc'
      ? originalData.title
      : type === 'article'
  ...
  const fullSemanticEmbedding =
    type === 'doc'
      ? await generateEmbeddings(fullPlainText, 'SEMANTIC_SIMILARITY')
      : type === 'article'
      ? originalData.semanticEmbedding
      : null;
  const fullRetrievalEmbedding =
    type === 'doc'
      ? await generateEmbeddings(fullPlainText, 'RETRIEVAL_DOCUMENT')
      : type === 'article'
